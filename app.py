...
# 設定受付
width = st.number_input("幅 (px)", min_value=50, max_value=3000, value=400)
height = st.number_input("高さ (px)", min_value=50, max_value=3000, value=600)
suffix = st.text_input("ファイル名サフィックス", value="_thumb")
format = st.selectbox("保存形式", ["PNG", "JPEG"])
keep_aspect = st.checkbox("縦横比を維持する", value=True)
if format == "JPEG":
    jpeg_quality = st.slider("JPEG品質", 50, 95, 85)
else:
    jpeg_quality = None
text = st.text_input("サムネ文字", value="test")
position = st.selectbox("文字位置", ["top", "center", "bottom"])
